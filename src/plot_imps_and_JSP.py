...
jetto=ps.Getexp(pulse,dda='JSP', uid='jhb3479', seq=514)

dtypes =  ['ZEFF']
for dty in dtypes:
    jetto.add_dty(dty)
ddaimp = {'SSP1':'BE','SSP2':'NE','SSP3':'W','SSP4':'NI'}
impdda = {ddas[k] : k for k in ddas}#assuming 1-1 mapping 
for dda in ddaimp.keys(): 
    jetto.add_dty('DZ', dda=dda)

# get eq data from MMX in Provenance
#eq=ps.Eq(pulse,uid='mporad',seq=556)
...
if profs.zeffp is not None: 
    ...
    if 'JSP/ZEFF' in jetto.dty:
        ax.plot(
           jetto.x['JSP/ZEFF'][ps.gi(wsxp.t['JSP/ZEFF'],time+40.)],
           jetto.data['JSP/ZEFF'][ps.gi(jetto.t['JSP/ZEFF'],time+40.)],
           label=f'ZEFF JETTO',
           color = 'k',
           linestyle = '--'
           )
    if 'NI' in impdda:
        ax.plot(
           jetto.x[f"{impdda['NI']}/DZ"][ps.gi(jetto.t[f"{impdda['NI']}/DZ"],time+40.)],
           jetto.data[f"{impdda['NI']}/DZ"][ps.gi(jetto.t[f"{impdda['NI']}/DZ"],time+40.)],
           label=f'Ni JETTO',
           color = 'darkorange',
           linestyle = '--'
           )
...
for imp in profs.impurities:
    ...
    ax.plot(
           wsxp.x['ZEFF'][ps.gi(wsxp.t['ZEFF'],time+40.)],
           wsxp.data[jettozeff][ps.gi(wsxp.t[jettozeff],time+40.)],
           label=f'{imp} JETTO {ps.gi(wsxp.t[jettozeff],time+40.)}, t= {time+40.:.3f}'
           )
    ax.set_xlabel(r'$\rho_{tor}^{norm}$')
    ax.set_ylim([0,None])
    leg=ax.legend()                                                                                     
