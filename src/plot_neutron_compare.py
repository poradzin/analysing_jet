...
parser.add_argument('--compare2', type=str,
                    help='Second RunID to compare against (plotted with dotted lines)')
...
# Load comparison runs if provided
...
neutrons3, transp3 = None, None
if args.compare2:
    neutrons3 = ps.Neutrons(pulse, args.compare2)
    neutrons3.get_transp_neutrons()
    neutrons3.get_exp()
    transp3 = ps.Transp(pulse, args.compare2)
...
def safe_divide(x, y):
    """Returns x/y where y > 0, and 0 elsewhere."""
...
def cum_int(x, jacobian):
    ...
    return np.cumsum(x * jacobian, axis=1)

def thbt(transp, bnd):
    """TH/BT ratio as a function of time, integrated up to rho=bnd."""
...
def plot_both(ax, time1, data1, label, color, time2=None, data2=None, runid2=None):
    """Plot main run (solid) and optionally a comparison run (dashed)."""
    if data1 is None:
        return
...
plot_both(ax,
          neutrons.transp_time+40, neutrons.transp('NEUTX_DD'), 'Thermal DD', 'green',
          neutrons2.transp_time+40 if neutrons2 else None,
          neutrons2.transp('NEUTX_DD') if neutrons2 else None,
          args.compare)

plot_both(ax,
          neutrons.transp_time+40, neutrons.transp('BTNTS_DD'), 'Beam-target DD', 'blue',
          neutrons2.transp_time+40 if neutrons2 else None,
          neutrons2.transp('BTNTS_DD') if neutrons2 else None,
          args.compare)
...
###################################################################
# Tab 3: Thermal/beam ratio to total
###################################################################
fig = plt.figure()
ax = fig.add_subplot(111)
add_strap(ax, args)
...
###################################################################
# Tab 4: TH/BT ratio (volume average)
###################################################################
fig = plt.figure()
ax = fig.add_subplot(111)
add_strap(ax, args)
...
###################################################################
# Tab 5: TH/BT ratio (volume average + radial)
###################################################################
bnd = 0.2
fig = plt.figure()
ax = fig.add_subplot(111)
...
color1 = 'black'
if neutrons.transp('BTNTS') is not None:
    ax.plot(neutrons.transp_time+40, safe_divide(neutrons.transp('NEUTX'), neutrons.transp('BTNTS')),
            color=color1, linewidth=2, linestyle='--', label="TH/BT average")
if neutrons.check_signal('BTNTX'):
    ax.plot(neutrons.transp_time+40, thbt(neutrons, bnd),
            color=color1, linewidth=2, label=r"TH/BT ($\rho<$"+f"{bnd})")

if neutrons2 is not None:
    color2 = 'orange'
    if neutrons2.transp('BTNTS') is not None:
    ...
    if neutrons2.check_signal('BTNTX'):
        ax.plot(neutrons2.transp_time + 40,
                thbt(neutrons2, bnd),
                color=color2, linewidth=2,
                label=rf"TH/BT ($\rho<{bnd}$) ({args.compare})")

if neutrons3 is not None:
    color3 = 'green'
    if neutrons3.transp('BTNTS') is not None:
        ax.plot(neutrons3.transp_time + 40,
                ...
                label=f"TH/BT average ({args.compare2})")
    if neutrons3.check_signal('BTNTX'):
        ax.plot(neutrons3.transp_time + 40,
                thbt(neutrons3, bnd),
                color=color3, linewidth=2,
                label=rf"TH/BT ($\rho<{bnd}$) ({args.compare2})")
...
leg = ax.legend()
leg.set_draggable(True)
win.addPlot('TH/BT ratio (radial)', fig)
